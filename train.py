...
def train(
    ...
    frn=False,
    style="./styles/style.jpg"):

    print(f"start processing for model: {output_file}")
    ...
    with torch.cuda.device(gpu_device):
        ...
        for epoch in range(n_epochs):
            for sample in traindata:
                optimizer.zero_grad()
                # print("processing sample", datetime.datetime.now())
                ...
                with torch.no_grad():
                    ...
                    if global_step % 100 == 0 or last_iteration:
                        print("processing step ", global_step,  datetime.datetime.now())
